function [ AUC ] = calc_AUC_score( salMap, eyeMap, shufMap, numRandom )
...
[~,~,~,AUC] = perfcurve([1 zeros(1, size(localRan,2))],[localHum localRan],1);

% count = 0;
% for ii = 1:size(localRan,2)
%     if(localHum > localRan(:, ii))
%         count = count + 1;
%     end
% end
% AUC = count / size(localRan,2);
